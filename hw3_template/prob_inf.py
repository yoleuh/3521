...
def calc_global_joint_prob(model, variableValues):
  ...
  for var in model.vars:
        # Get the values of the parents of the current variable
        parent_values = {parent: variableValues[parent] for parent in model.varDist[var].parents}
        
        # Get the conditional probability of the current variable given its parents' values
        prob = read_cpt(model, var, parent_values)
        
        # Multiply the joint probability by the conditional probability
...
def calc_query_exact_brute(model, queryVar, queryVal, evidence):
  ...
  for jptEntry in truefalse_combination_iterator(model.vars):
    ...
    # Check if the entry matches the evidence
    if dict_issubset(jptEntry, evidence):
      pr_E += pr_entry
      # Check if the entry also matches the query
...
def recurse_calc_query_exact_tree(model, queryVar, evidence, variableValues, remainingCalc):
  ...
  if marginalize:
    ...
    if queryVar in model.varDist[var].parents:
      ...
      prVarGivenParents_T = read_cpt(model, var, variableValues)
      prQ_T *= prVarGivenParents_T
      variableValues[queryVar] = False
      prVarGivenParents_F = read_cpt(model, var, variableValues)
      prQ_F *= prVarGivenParents_F
      variableValues[queryVar] = None
      ...
      # (Reference solution is 3 lines of code.)
      variableValues[queryVar] = True
      prVar_T = read_cpt(model, var, variableValues)
      prQ_T *= prVar_T
      variableValues[queryVar] = False
      prVar_F = read_cpt(model, var, variableValues)
      prQ_F *= prVar_F
      variableValues[queryVar] = None
      ...
      pr = read_cpt(model, var, variableValues)
      prQ_T *= pr
      prQ_F *= pr
